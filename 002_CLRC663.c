...
u8 Clrc663_Recv(void)
{
    ...
    do {
        ...
        if (len > 0) {
            ...
            temp = Uart3_Read(0);// alway read index 0
            return temp;
        } else {
            delay_ms(10);
        }
...
void Clrc663_SendRegValue(u8 addr, u8 data)
{
    Clrc663_Send(addr);
    Clrc663_Send(data);
}

u8 Clrc663_RecvRegValue(u8 addr)
{
    Clrc663_Send(addr);
    return Clrc663_Recv();
}
...
void clrc663_read_fifo(u8* rx, u16 len) {
    u8 addr = CLRC630_REG_FIFODATA | 0x80;
...
void clrc663_cmd_read_E2(u16 address, u16 length) {
  u8 parameters[3] = {(u8) (address >> 8), (u8) (address & 0xFF), length};
  clrc663_flush_fifo();
...
void clrc663_cmd_load_reg(u16 address, u8 regaddr, u16 length) {
  u8 parameters[4] = {(u8) (address >> 8), (u8) (address & 0xFF), regaddr, length};
  clrc663_flush_fifo();
...
void clrc663_cmd_auth(u8 key_type, u8 block_address, const u8* card_uid) {
  clrc663_cmd_idle();
  u8 parameters[6] = {key_type, block_address, card_uid[0], card_uid[1], card_uid[2], card_uid[3]};
  clrc663_flush_fifo();
...
void clrc663_clear_irq0() {
  clrc663_write_reg(CLRC630_REG_IRQ0, (u8) ~(1<<7));
}
void clrc663_clear_irq1() {
  clrc663_write_reg(CLRC630_REG_IRQ1, (u8) ~(1<<7));
}
u8 clrc663_irq0() {
...
u16 clrc663_timer_get_value(u8 timer) {
  u16 res = clrc663_read_reg(CLRC630_REG_T0COUNTERVALHI + (5 * timer)) << 8;
...
u8 clrc663_iso14443a_select(u8* uid, u8* sak) {
  ...
  // u8 uid_size = (atqa & (0b11 << 6)) >> 6;
  // u8 bit_frame_collision = atqa & 0b11111;
  ...
  for (cascade_level=1; cascade_level <= 3; cascade_level++) {
    ...
    for (collision_n=0; collision_n < 32; collision_n++) {
      ...
      u8 irq0 = clrc663_irq0();
      u8 error = clrc663_read_reg(CLRC630_REG_ERROR);
      u8 coll = clrc663_read_reg(CLRC630_REG_RXCOLL);
      ...
      if (irq0 & CLRC630_IRQ0_ERR_IRQ) {  // some error occured.
        ...
        if (error & CLRC630_ERROR_COLLDET) {
          ...
          if (coll & (1<<7)) {
            ...
            u8 choice_pos = known_bits + collision_pos;
            u8 selection = (uid[((choice_pos + (cascade_level-1)*3)/8)] >> ((choice_pos) % 8))&1;
      ...
      u8 rx_len = clrc663_fifo_length();
      u8 buf[5];  // Size is maximum of 5 bytes, UID[0-3] and BCC.
    ...
    u8 bcc_val = uid_this_level[4];  // always at position 4, either with CT UID[0-2] or UID[0-3] in front.
    u8 bcc_calc = uid_this_level[0]^uid_this_level[1]^uid_this_level[2]^uid_this_level[3];
    if (bcc_val != bcc_calc) {
    ...
    u8 sak_len = clrc663_fifo_length();
    if (sak_len != 1) {
      return 0;
    }
    u8 sak_value;
...
u8 clrc663_communicate(u8* tx_buffer, u8 tx_buffer_len, u8* rx_buffer){
  ...
  u8 rx_buffer_len = clrc663_fifo_length();
...
u8 clrc663_MF_auth(const u8* uid, u8 key_type, u8 block) {
  ...
  u8 status = clrc663_read_reg(CLRC630_REG_STATUS);
...
u8 clrc663_MF_read_block(u8 block_address, u8* dest) {
  ...
  u8 send_req[2] = {CLRC630_MF_CMD_READ, block_address};
  ...
  u8 buffer_length = clrc663_fifo_length();
  u8 rx_len = (buffer_length <= 16) ? buffer_length : 16;
...
u8 clrc663_MF_write_block(u8 block_address, const u8* source) {
  ...
  u8 send_req[2] = {CLRC630_MF_CMD_WRITE, block_address};
...
void clrc663_MF_example_dump() {
  u16 atqa = clrc663_iso14443a_REQA();
